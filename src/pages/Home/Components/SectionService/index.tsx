...
export const SectionService = () => {
  ...
  const renderCard = (
    service: { id: number; title: string; description: string; route?: string },
    index: number
  ) => (
    <div
      key={service.id}
  ...
  const servicesToShow = allServices ? content.services : content.services.slice(0, 2)

  return (
    <section className="w-full min-h-[40rem] h-auto flex items-end justify-center" id={`${language === 'pt-br' ? 'servios' : 'services'}`}>
      <div className="container px-8 grid grid-cols-8 gap-4 py-14">
        ...
        <div className="col-span-8 flex items-center justify-center mt-6">
          <Button
            className="w-46 py-5 bg-blue-950 font-semibold"
